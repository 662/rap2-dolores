  ...
  render() {
    ...
    let mod =
      repository && repository.modules && repository.modules.length
        ...
        : {};
    let itf = mod.interfaces && mod.interfaces.length ? mod.interfaces.find(item => item.id === +params.itf) || mod.interfaces[0] : {};

    const pushState = history.location.state;
    let isPushState = false;
    if (pushState && pushState.properties) {
      itf.properties = [...pushState.properties];
      itf.properties.forEach(p => {
        p.interfaceId = itf.id;
      ...
      <article className="RepositoryEditor">
        <div className="header">
          ...
          <div className="toolbar">
            {/* 编辑权限：拥有者或者成员 */}
            {isOwned || isJoined ? (
              <span className="fake-link edit" onClick={e => this.setState({ update: true })}>
                <GoPencil /> 编辑
              </span>
            ) : null}
            ...
            <Link to={`${serve}/app/plugin/${repository.id}`} target="_blank" className="api">
              <GoPlug /> 插件
            </Link>
            <Link to={`${serve}/repository/get?id=${repository.id}`} target="_blank" className="api">
              <GoDatabase /> 数据
            </Link>
            <Link to={`${serve}/test/test.plugin.jquery.html?id=${repository.id}`} target="_blank" className="api">
              <GoJersey /> 测试
            </Link>
          </div>
        ...
        <div className="body">
          ...
          <div className="InterfaceWrapper">
            ...
            <InterfaceEditor editable={isPushState} itf={itf} properties={properties} mod={mod} repository={repository} />
          </div>
        </div>
      </article>
    );
